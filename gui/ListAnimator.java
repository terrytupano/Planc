...
/**
 * Animates moving list cells out of the way for a potential drop. This decorator completely over-paints the target
 * JList, optionally painting a dragged item and animating creation of a space for the dragged item to be dropped.
 * Thanks to Neil Cochran/keilly for a base visualization: http://jroller.com/page/swinguistuff?entry=animated_jlist
 */
public abstract class ListAnimator extends AbstractComponentDecorator {
	...
	public synchronized void endDrag(Point where) {
		...
		if (toIndex != -1 && toIndex != fromIndex) {
			...
			newBounds.put(new Integer(toIndex), bounds.get(new Integer(fromIndex)));
			if (fromIndex < toIndex) {
				for (int i = fromIndex + 1; i <= toIndex; i++) {
					newBounds.put(new Integer(i - 1), bounds.get(new Integer(i)));
				}
			} else {
	...
	/**
	 * Simple JList-local drag/drop handler. Invokes the animator according to user input. A similar method could be
	 * used to accept drags originating outside of the JList.
	 */
	...
	private static final List DATA = new ArrayList(Arrays.asList(new Object[]{"Happy", "Bashful", "Grumpy", "Sneezy",
			"Dopey", "Sleepy", "Doc", "Snow White", "Prince Charming", "Wicked Witch",}));

	/**
	 * Throw up a frame to demonstrate the animator at work. Funkify the list renderer to demonstrate that customized
	 * renderers are handled properly.
	 */
	public static void main(String[] args) {
		...
		list.setCellRenderer(new DefaultListCellRenderer() {
			public Component getListCellRendererComponent(JList list, Object value, int index, boolean sel,
				...
				if ((index % 2) == 0) {
					int GRAY = 190;
					c.setBackground(new Color((GRAY + bg.getRed() * 2) / 3, (GRAY + bg.getGreen() * 2) / 3, (GRAY + bg
							.getBlue() * 2) / 3));
