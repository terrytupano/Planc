...
public class MixedExample extends JFrame {
	...
	public MixedExample() {
		...
		AttributiveCellTableModel ml = new AttributiveCellTableModel(80, 10) {
		...
		private final void changeColor(boolean isForeground) {
			...
			for (int i = 0; i < rows.length; i++) {
				...
				for (int j = 0; j < columns.length; j++) {
					...
					target = (target != cellAtt.getForeground(row, column)) ? null : target;
					reference = (reference != cellAtt.getBackground(row, column)) ? null : reference;
				}
			}
			String title;
			if (isForeground) {
		...
		FontPanel(final JTable table, final CellFont cellAtt) {
			...
			b_apply.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if ((rows.length < 1) || (columns.length < 1))
						return;
					Font font = new Font((String) name.getSelectedItem(), style.getSelectedIndex(), Integer
							.parseInt((String) size.getSelectedItem()));
